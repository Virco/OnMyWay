...
public class ActivityRecognitionManager implements ConnectionCallbacks, OnConnectionFailedListener{
    ...
    public boolean servicesAvailable(){
        ...
        if(resultCode == ConnectionResult.SUCCESS){
        	...
        	Dialog errorDialog = GooglePlayServicesUtil.getErrorDialog(resultCode, (ActionBarActivity)context, FIX_PLAY_SERVICES);
        	if(errorDialog != null){
        		errorDialog.show();
        	}
            return false;
    ...
    */
	public void connect(){
        if(servicesAvailable()){	// Check for Google Play services
	        ...
	        if(!connecting){
	        	if(requestUpdates){
	        		Utils.showInfoMessage((ActionBarActivity)context, "Connecting...");
	        	}
	...
	public void onConnected(Bundle connectionHint){
    	connecting = false;
    	if(requestUpdates){
    		Utils.showConfirmMessage((ActionBarActivity)context, "Connected");
    		
    		// send request
    		recognitionClient.requestActivityUpdates(DETECTION_INTERVAL, recognitionIntent);
    	}else{
    		Utils.showConfirmMessage((ActionBarActivity)context, "Disconnected");
    		// cancel updates
	...
	public void onConnectionFailed(ConnectionResult result){
        ...
        if(result.hasResolution()){
            try{
                result.startResolutionForResult((ActionBarActivity)context, FIX_PLAY_SERVICES);
                // Thrown if Google Play services canceled the original PendingIntent
            }catch(SendIntentException e){
                // display an error message
            	Utils.showAlertMessage((ActionBarActivity)context, "Connection failed");
         ...
         */
            ...
            Dialog dialog = GooglePlayServicesUtil.getErrorDialog(result.getErrorCode(), (ActionBarActivity)context, FIX_PLAY_SERVICES);
            if(dialog != null){
                dialog.show();
            }
        }
